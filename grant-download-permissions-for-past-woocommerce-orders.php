 ...
 * Plugin URI:  https://github.com/nexitomedia/grant-download-permissions-for-past-woocommerce-orders
 * Description: This plugin grants downloads permissions like WooCommerce 2.6.x, granting permissions for new files added to a downloadable product. Note that this plugin performs heavy database queries and does not scale. For this reason it has been removed from WooCommerce core. The code has been updated for HSOP and PHP 8.3 compatibility.
 * Author:      Tom Wilson
...
namespace Grant_Download_Permissions;
                    ...
                    if ( ! empty( $new_download_ids ) ) {
                        foreach ( $new_download_ids as $download_id ) {
                            if ( apply_filters( 'woocommerce_process_product_file_download_paths_grant_access_to_new_file', true, $download_id, $product_id, $order ) ) {
                                ...
                                if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT 1 FROM {$wpdb->prefix}woocommerce_downloadable_product_permissions WHERE order_id = %d AND product_id = %d AND download_id = %s", $order->get_id(), $product_id, $download_id ) ) ) {
...
add_action( 'admin_init', [ 'Grant_Download_Permissions\WooCommerce_Legacy_Grant_Download_Permissions', 'get_instance' ] );
